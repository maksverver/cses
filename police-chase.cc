...
//
// The size of the minimum cut is simply equal to the maximum flow in the
// network. We also need to report the edges in the minimum cut. These we can
// find by looking at the vertices reachable in the residual graph. Any edge
// going from a reachable to a nonreachable vertex is part of the minimum cut.
...
int main() {
  ...
  while (E--) {
    ...
    edges.push_back(Edge(w, v, 0, 1));
  }

  MinCostMaxFlow mcmf;
  cout << mcmf(edges, 0, V - 1).second << '\n';

  // Calculate reachability from the source in O(V * E) time. This is similar
  // to Bellman-Ford above.
  vector<char> reachable(V, false);
  reachable[0] = true;
  bool changed = true;
  while (changed) {
    changed = false;
    for (const Edge &e : edges) {
      if (e.flow < e.cap && reachable[e.src] && !reachable[e.dst]) {
        reachable[e.dst] = true;
        changed = true;
      }
      if (e.flow > 0 && reachable[e.dst] && !reachable[e.src]) {
  ...
  // Now print the edges in the minimum cut.
  for (const Edge &e : edges) {
    if (reachable[e.src] && !reachable[e.dst]) {
      assert(e.flow == e.cap);
      cout << e.src + 1 << ' ' << e.dst + 1 << '\n';
    }
  }
}
